...
class DPL_AUC_PCBM(DPL_AUC):
    ...
    def forward(self, x):
        ...
        pred_embeddings = self._sample_gaussian_tensors(latents, logsigma, 100)
        concept_logit, concept_prob = self._compute_distance(pred_embeddings, z_tot)
        ...
        # if not self.senn:
        squeezed_c_logits = torch.unsqueeze(concept_prob[..., 1], dim=-1)
