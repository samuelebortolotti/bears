...
def create_w_to_y():
    three_bits_or = torch.cat((torch.zeros((8, 1)), torch.ones((8, 1))), dim=1).to(
        dtype=torch.float
    )
    three_bits_or[0] = torch.tensor([1, 0])

    six_bits_or = torch.cat((torch.zeros((64, 1)), torch.ones((64, 1))), dim=1).to(
        dtype=torch.float
    )
    six_bits_or[0] = torch.tensor([1, 0])

    and_not_for_stop = torch.tensor([[0, 1], [0, 1], [1, 0], [0, 1]], dtype=torch.float)
...
class DPL(nn.Module):
    ...
    def proglob_pred(self):
        ...
        labels_4 = torch.sigmoid(self.pred_5(self.concepts_labeled[:, :, 0])).view(
            -1, 1
        )
