...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return encoded_1, encoded_2, torch.ones(1)  # real_fake
...
class image_fcc_conceptizer(AutoEncoder):
    ...
    def __init__(
        self, din, nconcept, nconcept_labeled, cdim, sparsity, senn
    ):
        ...
        self.nconcept_labeled = (
            nconcept_labeled  # Number of unknown concepts
        )
        ...
        if senn == True:
            ...
            self.enc1_original = nn.Linear(
                self.din, self.nconcept_labeled
            )
            self.enc1 = nn.Linear(self.din, 512)
            self.enc_middle_f = nn.Linear(512, 3 * 2)
            self.enc_middle_s = nn.Linear(512, 6 * 2)
            self.enc_middle_l = nn.Linear(512, 6 * 2)
            self.enc_middle_r = nn.Linear(512, 6 * 2)
            # self.enc_final = nn.Linear(256, self.nconcept_labeled * 2)
            self.enc2 = nn.Linear(
                self.din, self.nconcept - self.nconcept_labeled
            )
    ...
    def encode(self, x):
        ...
        if self.senn == True:
            ...
            topval = topval.expand(
                encoded.shape[1], encoded.shape[0]
            ).permute(1, 0)
            ...
            # T = 2.5
            logits_c = self.enc1(p)
            logits_c = self.relu(logits_c)
            logits_c = self.drop(logits_c)
            logits_c_f = self.enc_middle_f(logits_c)
            logits_c_s = self.enc_middle_s(logits_c)
            logits_c_l = self.enc_middle_l(logits_c)
            logits_c_r = self.enc_middle_r(logits_c)

            # concatenate them
            logits_c = torch.cat(
                [logits_c_f, logits_c_s, logits_c_l, logits_c_r],
                dim=1,
            )
            ...
            logits_c = self.drop(logits_c)
            logits_c = self.relu(logits_c)
            logits_c = self.enc_final(logits_c)
            """
            logits_split = torch.split(logits_c, 2, dim=-1)

            all_logits = []
            for i in range(len(logits_split)):
                logit1 = torch.softmax(logits_split[i], dim=-1)[
                    :, 1
                ].view(-1, 1)
                all_logits.append(logit1)
            encoded_1 = torch.cat(all_logits, dim=-1)
            ...
            topval = topval.expand(
                encoded_2.shape[1], encoded_2.shape[0]
            ).permute(1, 0)
            comp = (encoded_2 >= topval).to(encoded_2)
            encoded_2 = comp * encoded_2

            # reshape for the following process
            encoded_2 = encoded_2.reshape(
                [encoded_2.shape[0], encoded_2.shape[1], 1]
            )
...
class image_cnn_conceptizer(AutoEncoder):
    ...
    def __init__(
        ...
        self.add_bias = False
        self.dout = int(
            np.sqrt(din) // 4 - 3 * (5 - 1) // 4
        )  # For kernel = 5 in both, and maxppol stride = 2 in both
        ...
        )  # b, 10, din - (k -1),din - (k -1)
        # after pool layer (functional)                        # b, 10,  (din - (k -1))/2, idem
        self.conv2_1 = nn.Conv2d(
            10, self.nconcept_labeled, kernel_size=5
        )  # b, 10, (din - (k -1))/2 - (k-1), idem
        self.conv2_2 = nn.Conv2d(
            10, self.nconcept - self.nconcept_labeled, kernel_size=5
        )  # b, 10, (din - (k -1))/2 - (k-1), idem
        self.linear1 = nn.ModuleList()
        for i in range(self.nconcept_labeled):
            self.linear1.append(nn.Linear(self.dout**2, self.cdim))
        self.linear2 = nn.Linear(
            self.dout**2, self.cdim
        )  # b, nconcepts, cdim

        # Decoding
        self.unlinear = nn.Linear(
            self.cdim, self.dout**2
        )  # b, nconcepts, dout*2
        self.deconv3 = nn.ConvTranspose2d(
            nconcept, 16, 5, stride=2
        )  # b, 16, (dout-1)*2 + 5, 5
        self.deconv2 = nn.ConvTranspose2d(
            16, 8, 5
        )  # b, 8, (dout -1)*2 + 9
        self.deconv1 = nn.ConvTranspose2d(
            8, nchannel, 2, stride=2, padding=1
        )  # b, nchannel, din, din
    ...
    def encode(self, x):
        ...
        for fc in self.linear1:
            encoded_1.append(
                fc(
                    p_1.view(
                        -1, self.nconcept_labeled, self.dout**2
                    )[:, cnt]
                )
            )
            cnt = cnt + 1
        ...
        # compute unknown concepts
        encoded_2 = self.linear2(
            p_2.view(
                -1,
                self.nconcept - self.nconcept_labeled,
                self.dout**2,
            )
        )
        encoded_2 = encoded_2.reshape(
            [encoded_2.shape[0], encoded_2.shape[1]]
        )
    ...
    def decode(self, z1_list, z2):
        ...
        q = self.unlinear(z).view(
            -1, self.nconcept, self.dout, self.dout
        )
...
class PCBMConceptizer(image_fcc_conceptizer):
    def __init__(
        self,
        din,
        nconcept,
        ...
        super(PCBMConceptizer, self).__init__(
            din, nconcept, nconcept_labeled, cdim, sparsity, senn
        )

        self.latent_dim = 16

        self.positives = torch.nn.Parameter(
            torch.randn(1, int(21), 16), requires_grad=True
        )
        self.negatives = torch.nn.Parameter(
            torch.randn(1, int(21), 16), requires_grad=True
        )

        self.negative_scale = 1 * torch.ones(1, device=device)
        self.shift = torch.ones(0, device=device)

        self.dense_logvar = nn.Linear(
            in_features=self.din,
            out_features=self.latent_dim * self.nconcept_labeled,
        )
    ...
    def encode(self, x):
        ...
        T = 2.5
        logits_c = self.identity(p)
        logits_c = self.drop(logits_c)
        logits_enc = self.enc1_original(logits_c) / T

        encoded_1 = torch.sigmoid(logits_enc)

        mu, logvar = self.dense_mu(logits_c), self.dense_logvar(
            logits_c
        )

        mu = torch.stack(
            torch.split(mu, self.latent_dim, dim=-1), dim=1
        )
        logvar = torch.stack(
            torch.split(logvar, self.latent_dim, dim=-1), dim=1
        )
        ...
        return encoded_1, encoded_2, mu, logvar

    def forward(self, x):
        # compute concepts
        encoded_1, encoded_2, mu, logvar = self.encode(x)
        return encoded_1, encoded_2, [mu, logvar]
