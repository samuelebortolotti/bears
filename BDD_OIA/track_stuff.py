...
def get_dfs_merged(seeds, category, lambda_h, lambda_kl, train="train", pcbm=False):
    dfs = []
    for seed in seeds:
        if pcbm:
            df = pd.read_csv(
...
def get_dfs_single(seed, n_model, category, lambda_h, lambda_kl, train="train"):
    df = pd.read_csv(
...
def compute_f1(df):
    ...
    for i in range(4):
        ...
        y_cfs.append(confusion_matrix(y_true[:, i], y_pred[:, i], normalize="true"))
        to_rtn[f"F1 of Label {i}"] = f1_value
        ece_value = produce_ece_curve(y_prob[:, i], y_pred[:, i], y_true[:, i])
    ...
    for i in range(21):
        ...
        c_pred.append(np.round(prob_C[:, i]).astype(int).reshape(-1, 1))
        cfs.append(
            confusion_matrix(c_true[:, i], np.round(prob_C[:, i]), normalize="true")
        )
        ece_value = produce_ece_curve(
            prob_C[:, i], np.round(prob_C[:, i]).astype(int), c_true[:, i].astype(float)
        )
    ...
    to_rtn["F1 all concepts"] = f1_score(all_c_true, all_c_pred, average="macro")
...
def _populate_bins(confs, preds, labels, num_bins: int):
    ...
    for i, (confidence, prediction, label) in enumerate(zip(confs, preds, labels)):
        try:
...
def expected_calibration_error(confs, preds, labels, num_bins: int = 10):
...
def get_stat(
    ...
    df_bir_full = get_dfs_merged([seed], category, lambda_h, lambda_kl, train=train)
    df_bir_list = [get_f1_per_dict(df_bir_full[0], is_list=False)]
    bir_dict = filter_dict(to_dict(pd.concat(df_bir_list, ignore_index=True)), keys)

    bir_full_to_log = {}
    for key, value in bir_dict.items():
        bir_full_to_log[f"Factorized_on_{train}_{key}"] = value
    bir_full_to_log = from_list_to_value(convert_to_json_serializable(bir_full_to_log))
    ...
    try:
        for i in range(n_model):
            df_bir_f = get_dfs_single(seed, i, category, lambda_h, lambda_kl, train)
            ...
            bir_full_to_log = {}
            for key, value in bir_dict.items():
                bir_full_to_log[f"Single_n_{i}_on_{train}_{key}"] = value
