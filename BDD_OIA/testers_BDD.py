...
class ClassificationTester:
    def __init__(self, model, args, device):
        ...
        self.lambd = (
            args.theta_reg_lambda if ("theta_reg_lambda" in args) else 1e-6
        )  # for regularization strenght
        self.eta = (
            args.h_labeled_param if ("h_labeled_param" in args) else 0.0
    ...
    def validate(self, val_loader, epoch, fold=None, name=""):
        ...
        for i, (inputs, targets, concepts) in enumerate(val_loader):
            ...
            h_loss, hh_labeled = self.concept_learning_loss_for_weak_supervision(
                inputs, all_losses, concepts, self.args.cbm, self.args.senn, epoch
            )
    ...
    def compute_accuracy_f1(self, y_pred, y_true):
        ...
        for i in range(len(y_preds_list)):
            y_preds_list[i] = torch.argmax(y_preds_list[i], dim=1).unsqueeze(-1)
    ...
    def accuracy(self, output, target, topk=(1,), numpy=False):
        if numpy:
            ...
            correct = pred == target  # (pred == target.reshape((-1, 1)))

            # If topk = (1,5), then, k=1 and k=5
            res = []
            for k in topk:
                correct_k = np.sum(correct[:, :k].any(axis=1).astype(float))
    ...
    def concept_learning_loss_for_weak_supervision(
            ...
            if (-1) in self.args.which_c:
                ...
                for i in range(21):
                    if i in self.args.which_c:
                        labeled_loss = (
                            F.binary_cross_entropy(
                                hh_labeled_list[0, i], concepts[0, i].to(self.device)
                            )
                            / L
                        )
                        for j in range(1, len(hh_labeled_list)):
        ...
        if not senn:
            # save loss (only value) to the all_losses list
            all_losses["labeled_h"] = self.eta * labeled_loss.data.cpu().numpy()
    ...
    def entropy_loss(self, pred_c, all_losses, epoch):
        ...
        total_ent = -avg_c[0] * torch.log(avg_c[0]) - (1 - avg_c[0]) * torch.log(
            1 - avg_c[0]
        )
        total_ent /= np.log(2)
        for i in range(1, 21):
            ent_i = -avg_c[i] * torch.log(avg_c[i]) - (1 - avg_c[i]) * torch.log(
                1 - avg_c[i]
            )
            ent_i /= np.log(2)
    ...
    def test_and_save_csv(
        self, test_loader, save_file_name, fold=None, dropout=False, pcbm=False
    ):
    ...
    def get_concept_probability_factorized_ensemble(self, ensemble, loader):
        ensemble_c_prb = []
        ...
        for model in ensemble:
            ...
            for i, data in enumerate(loader):
                ...
                if i == 0:
                    ...
                    c_prb = np.concatenate(
                        [c_prb, concept.detach().cpu().numpy()], axis=0
                    )
            ...
            gt_factorized = [
                np.where(row == gt_factorized_max[i])[0] for i, row in enumerate(c_true)
            ]
...
class Frequentist(ClassificationTester):
    ...
    def frequentist_batch_prediction(self, batch_samples, apply_softmax=False):
    ...
    def frequentist_prediction(self, loader, apply_softmax=False):
        ...
        for i, data in enumerate(loader):
            ...
            (label_prob, concept_prob) = (  # (256, 4)  # (256, 21)
                self.frequentist_batch_prediction(images, apply_softmax)
            )
            ...
            if i == 0:
                ...
                pc_pred = np.concatenate([pc_pred, concept_prob], axis=0)
    ...
    def worlds_probability(
        ...
        # get the prediction
        y_true, c_true, y_pred_org, pc_pred = self.frequentist_prediction(
        ...
        fstop_prob = compute_forward_stop_prob(pc_pred_ext)  # data, possibleworlds
        left_prob = compute_left(pc_pred_ext)  # data, possibleworlds
        right_prob = compute_right(pc_pred_ext)  # data, possibleworlds
        y_prob = compute_output_probability(y_pred)  # data, possibleworlds
        ...
        for prob in w_probs:
            ...
            w_predictions_prob_value.append(np.max(prob, axis=-1))  # data, 1

        # compute the ground_truth (which is simply the binary representation up of that slice)
        fstop_ground = compute_forward_stop_groundtruth(c_true)  # data, 1
        ...
        y_predictions_prob = np.concatenate(y_preds_prob_list[:4], axis=-1)
...
class MCDropout(ClassificationTester):
    ...
    def test_and_save(self, test_loader, save_file_name, fold=None):
        ...
        for i in range(self.num_mc_samples):
            current_model_name = add_previous_dot(save_file_name, f"_montecarlo_{i}")
            super().test_and_save(test_loader, current_model_name, fold)
    ...
    def _montecarlo_dropout_single_batch(
        ...
        # activate the double return
        self.model.return_both_concept_out_prob = True
        output_list = [self.model(batch_samples) for _ in range(num_mc_samples)]  # 30
        # deactivate the double return
        self.model.return_both_concept_out_prob = False
        self._deactivate_dropout()

        label_prob = [lab.detach().cpu().numpy() for lab, _ in output_list]  # 30
        concept_prob = [
            concept.detach().cpu().numpy() for _, concept in output_list
        ]  # 30
    ...
    def mc_dropout_predictions(
        ...
        for i, data in enumerate(loader):
            ...
            # Call MC Dropout
            (label_prob_ens, concept_prob_ens) = (  # (nmod, 256, 4)  # (nmod, 256, 21)
                self._montecarlo_dropout_single_batch(
            ...
            if i == 0:
                ...
                pc_pred = np.concatenate([pc_pred, concept_prob_ens], axis=1)
    ...
    def worlds_probability(
        ...
        fstop_prob = compute_forward_stop_prob(pc_pred)  # data, possibleworlds
    ...
    # MC DROPOUT
    def test_and_save_csv(self, test_loader, save_file_name, fold=None, pcbm=False):
        ...
        for i, (inputs, targets, concepts) in enumerate(test_loader):
            ...
            # compute output
            (label_prob_ens, concept_prob_ens) = (  # (nmod, 256, 4)  # (nmod, 256, 21)
                self._montecarlo_dropout_single_batch(inputs, 30, False)
            )
            ...
            if self.cuda:
                output, pc_prob = output.cuda(self.device), pc_prob.cuda(self.device)
        ...
        for _ in range(self.num_mc_samples):
            ...
            for i, data in enumerate(loader):
                ...
                if i == 0:
                    ...
                    c_prb = np.concatenate([c_prb, concept_prob], axis=1)

            # get the worlds probabilities by computing the matmultiplication
            fstop_prob = compute_forward_stop_prob(c_prb)  # data, possibleworlds
...
class LaplaceBayes(ClassificationTester):
    ...
    def setup(
        ...
        self.laplace_model = self.laplace_approximation(train_loader, val_loader)
    ...
    def test_and_save(self, test_loader, save_file_name, fold=None):
        ...
        for i in range(self.num_mc_samples):
            current_model_name = add_previous_dot(save_file_name, f"_laplace_{i}")
    ...
    def laplace_approximation(self, train_loader, val_loader):
        ...
        def new_model_copy(to_be_copied):
            ...
            parametrizer1 = dfc_parametrizer(
                2048, 1024, 512, 256, 128, self.nconcepts, self.theta_dim, layers=4
            )

            if self.cbm == True:
                aggregator = CBM_aggregator(
                    self.concept_dim, self.nclasses, self.nconcepts_labeled
                )
            else:
                aggregator = additive_scalar_aggregator(self.concept_dim, self.nclasses)
            ...
            model.load_state_dict(copy.deepcopy(to_be_copied.state_dict()))
            return model
        ...
        class WrapperDataLoader(DataLoader):
            ...
            def __iter__(self):
                ...
                original_iterator = super(WrapperDataLoader, self).__iter__()
        ...
        class WrapperModel(nn.Module):
            ...
            def forward(self, input_batch):
                ...
                concept_p_x, h_x, _ = self.original_model.conceptizer(input_batch)
                ...
                return torch.cat((original_output, concept_p_x), dim=1)
            ...
            def get_ensembles(self, la_model, n_models):
                ...
                for i, mp in enumerate(self.model_possibilities):
                    _vector_to_parameters(mp, la_model.model.last_layer.parameters())
                    ensembles.append(
                        new_model_copy(la_model.model.model.original_model)
                    )
                    if i == n_models - 1:
                        break
        ...
        # wrap the model
        la_model = WrapperModel(new_model_copy(self.model), self.model.device)
        ...
        return self._fit_la_model(la, la_training_loader, la_val_loader)

    # fit the laplace model
    def _fit_la_model(self, la, la_training_loader, la_val_loader):
        fprint("Doing Laplace fit...")
        la.fit(la_training_loader)
        la.optimize_prior_precision(method="marglik", val_loader=la_val_loader)
    ...
    def test_and_save_csv(self, test_loader, save_file_name, fold=None, pcbm=False):
        ...
        for i, (inputs, targets, concepts) in enumerate(test_loader):
            ...
            _ = self.laplace_single_prediction(self.laplace_model, inputs, 5, 21, False)
    ...
    def laplace_prediction(
        ...
        for i, data in enumerate(loader):
            ...
            _ = self.laplace_single_prediction(
                laplace_model, images, output_classes, num_concepts, apply_softmax
            )
    ...
    def laplace_single_prediction(
        ...
        recovered_pred = self.recover_predictions_from_laplace(
            pred, sample_batch.shape[0], output_classes, num_concepts, apply_softmax
        )
    ...
    def recover_predictions_from_laplace(
        ...
        # Recovering shape
        ys = la_prediction[:, :output_classes]  # take all until output_classes
        pCS = la_prediction[
            :, output_classes:
...
class DeepEnsembles(ClassificationTester):
    ...
    def test_and_save(self, test_loader, save_file_name, fold=None):
        for i in range(len(self.ensemble)):
            current_model_name = add_previous_dot(save_file_name, f"_{self.name}_{i}")
    ...
    def test_and_save_csv(self, test_loader, save_file_name, fold=None, pcbm=False):
        ...
        for i, (inputs, targets, concepts) in enumerate(test_loader):
            ...
            # compute output
            (label_prob_ens, concept_prob_ens) = self._ensemble_single_predict(
            ...
            # prediction_criterion is defined in __init__ of "class GradPenaltyTrainer"
            loss = self.prediction_criterion(torch.tensor(output), targets)
    ...
    def save_model_params_all(self, save_path, separate_from_others, lambda_h):
    ...
    def _populate_pcx_dataset_knowledge_aware(self, model, pcx_loader, n_facts):
        ...
        FS_w_q = build_world_queries_matrix_complete_FS().to(self.model.device)
        ...
        for _, data in enumerate(pcx_loader):
            ...
            left_prob = compute_left(p_c_x_ext, numpy=False)  # data, possibleworlds
            right_prob = compute_right(p_c_x_ext, numpy=False)  # data, possibleworlds
            ...
            fstop_prob = fstop_prob * FS_w_q_transposed[label_indices[:, 0], :]
            left_prob = left_prob * L_w_q_transposed[label_indices[:, 1], :]
            right_prob = right_prob * R_w_q_transposed[label_indices[:, 2], :]
            ...
            w_fstop_tmp = list(
                map(lambda x: x.squeeze(0), torch.split(fstop_prob, 1, dim=0))
            )
            w_left_tmp = list(
                map(lambda x: x.squeeze(0), torch.split(left_prob, 1, dim=0))
            )
            w_right_tmp = list(
                map(lambda x: x.squeeze(0), torch.split(right_prob, 1, dim=0))
            )
    ...
    def _update_pcx_dataset(self, model, dataset, pcx_loader, batch_size):
        indexes = 0
    ...
    def _update_pcx_dataset_knowledge_aware(self, model, dataset, pcx_loader, n_facts):
    ...
    def give_full_worlds(self, model_itself_pc_x):
        ...
        f_prob = compute_forward_prob(p_c_x_ext, numpy=False)  # data, possibleworlds
        stop_prob = compute_forward_prob(p_c_x_ext, numpy=False)  # data, possibleworlds
        left_prob = compute_left(p_c_x_ext, numpy=False)  # data, possibleworlds
        right_prob = compute_right(p_c_x_ext, numpy=False)  # data, possibleworlds

        return f_prob, stop_prob, left_prob, right_prob

    def compute_pw_knowledge_filter(self, model_itself_pc_x, labels, wfs, wl, wR):

        p_c_x_ext = torch.unsqueeze(model_itself_pc_x, dim=0)

        fstop_prob = compute_forward_stop_prob(p_c_x_ext, numpy=False).to(
            self.model.device
        )  # data, possibleworlds
        ...
        w_left_tmp = list(map(lambda x: x.squeeze(0), torch.split(left_prob, 1, dim=0)))
        w_right_tmp = list(
            map(lambda x: x.squeeze(0), torch.split(right_prob, 1, dim=0))
        )
    ...
    def setup(
        ...
        if separate_from_others:
            self.FS_w_q = build_world_queries_matrix_complete_FS().to(self.model.device)
            self.L_w_q = build_world_queries_matrix_L().to(self.model.device)
            self.R_w_q = build_world_queries_matrix_R().to(self.model.device)
    ...
    def train_ensembles(
        ...
        fprint("Done!\nTotal length of the ensemble: ", len(self.ensemble))
    ...
    def train_single_model(
        ...
        early_stopper = EarlyStopper(patience=5, min_delta=0.001)  # prev 0.01
        ...
        for epoch in range(epochs):
            ...
            if self.args.wandb is not None:
                wandb.log(
                    {f"lr-model-{model_idx}": float(self.scheduler.get_last_lr()[0])}
                )
    ...
    def train_batch_single_model(
        ...
        def kl_paper(p_model: torch.tensor, p_rest: torch.tensor, k):
            ...
            kl_ew = torch.sum(p_model * torch.log(1 + (k - 1) * ratio), dim=1)
        ...
        # save loss (only value) to the all_losses list
        all_losses = {f"model {model_idx} prediction": pred_loss.cpu().data.numpy()}
        ...
        def entropy(probabilities: torch.tensor, concept=False):
            def concept_entropy(p):
                return -torch.sum(p * torch.log2(p)) / (len(p) * math.log(2))
            ...
            probabilities = probabilities / (1 + (probabilities.shape[1] * 1e-5))
            ...
            if concept:
                ...
                entropies = vmap(single_concept_entropy)(probabilities)
        ...
        if not separate_from_others:
            ...
            loss_adversarial = self.prediction_criterion(out_dict_adversarial, targets)
            ...
            # Adding entropy
            if model_idx == 0:
                loss = loss + (1 - entropy(model_itself_pc_x)) * lambda_h
                print("\tEntropy loss:", (1 - entropy(model_itself_pc_x)).item())
                all_losses.update(
            ...
            if model_idx > 0:
                if self.knowledge_aware_kl:
                    model_itself_pfs_x, model_itself_pleft_x, model_itself_right_x = (
                    ...
                    other_pf_mean = torch.mean(pf_list_ensemble, dim=0)  # .unsqueeze(0)

                    pstop_list_ensemble = torch.stack(pstop_list_ensemble)
                    ...
                    # mean right
                    prightx_list_ensemble = torch.stack(prightx_list_ensemble)
                    ...
                    with torch.no_grad():
                        ...
                        entropy_pc_x = entropy(model_itself_pc_x, True)
                        entropy_f = entropy(model_itself_pf_x, True)
                        entropy_s = entropy(model_itself_pstop_x, True)
                        entropy_l = entropy(model_itself_pleft_x, True)
                        entropy_r = entropy(model_itself_right_x, True)
                    ...
                    distance = distance + (1 - entropy(model_itself_pc_x)) * lambda_h
                    ...
                    pcx_list_ensemble = torch.stack(pcx_list_ensemble, dim=0)
                    pcx_list_ensemble = torch.mean(pcx_list_ensemble, dim=0)
                    ...
                    """
                    print(model_itself_pc_x.shape, pcx_list_ensemble.shape)
                    distance = lambda_h * kl_paper(
                        model_itself_pc_x, pcx_list_ensemble, len(self.ensemble) + 1
                    )
        ...
        for i, (inputs, targets, concepts) in enumerate(train_loader, 0):
            ...
            if self.nclasses > 4:
                ...
                prec1, _ = self.accuracy(
                    outputs.data, targets.data, topk=(1, self.nclasses)
                )
            else:
    ...
    def worlds_probability(
        ...
        y_true, c_true, y_pred, pc_pred = self.ensemble_predict(loader, apply_softmax)
...
class ClassificationTesterFactory:
    @staticmethod
    def get_model(name: str, model, args, device) -> ClassificationTester:
        if name == "frequentist":
            ...
            raise ValueError("The chosen model is not valid: chosen", name)
