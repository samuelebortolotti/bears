...
def convert_np_array_to_binary(array):
    return np.apply_along_axis(
        lambda row: int("".join(map(str, row[::-1])), 2), axis=1, arr=array.astype(int)
    )


def compute_world_probability(concepts_list, numpy=True, clip_value=1e-5):
    if not numpy:
        world_matrix = squeeze_last_dimensions(concepts_list[0], numpy)
        ...
        world_matrix = world_matrix.view(
            world_matrix.shape[0], world_matrix.shape[1], collapsed_dim
        )
        ...
        world_matrix = np.reshape(
            world_matrix, (world_matrix.shape[0], world_matrix.shape[1], collapsed_dim)
        )
...
def create_concepts_array(concepts_list, num_ones, i=2, numpy=True):
    ...
    shape = (n_models, batch_size) + ((1,) * i) + (2,) + ((1,) * (num_ones - i))

    slice_1 = tuple([slice(None), slice(None)] + [0] * i + [0] + [0] * (num_ones - i))
    slice_2 = tuple([slice(None), slice(None)] + [0] * i + [1] + [0] * (num_ones - i))
