...
def save_checkpoint(state, is_best, outpath, seed):
    ...
    if is_best:
        shutil.copyfile(filename, os.path.join(outpath, f"model_best-{seed}.pth.tar"))
...
class ClassificationTrainer:
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            if val_loader is not None:
                val_prec1 = 1
                val_prec1, last_loss = self.validate(val_loader, epoch + 1, pcbm)
    ...
    def concept_learning_loss_for_weak_supervision(
        ...
        if not senn:
            ...
            all_losses["labeled_h"] = labeled_loss.data.cpu().numpy() * self.eta
    ...
    def train_epoch(self, epoch, train_loader, val_loader=None, pcbm=False):
        ...
        for i, (inputs, targets, concepts) in enumerate(train_loader, 0):
            ...
            outputs, loss, loss_dict, hh_labeled, pretrained_out = self.train_batch(
    ...
    def validate(self, val_loader, epoch, fold=None, pcbm=False):
        ...
        for i, (inputs, targets, concepts) in enumerate(val_loader):
            ...
            h_loss, hh_labeled = self.concept_learning_loss_for_weak_supervision(
                inputs, all_losses, concepts, self.args.cbm, self.args.senn, epoch, pcbm
            )
