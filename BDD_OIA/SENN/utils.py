...
def explain_digit(model, x_raw, x, thresh=0.5, save_path=None):
    ...
    if save_path:
        plt.savefig(
            save_path + "_input.pdf", bbox_inches="tight", format="pdf", dpi=300
        )
    plt.show()
    y_pred = model(x)
    ...
    for i in range(model.dout):
        ...
        x_imask = _explain_class(
            model, x_raw, x, i, typ="pos", recompute=False, thresh=thresh
        )
        ...
        # Negative
        x_imask = _explain_class(
            model, x_raw, x, i, typ="neg", recompute=False, thresh=thresh
        )
...
class FeatureInput_Explainer:
    ...
    def explain(self, model, x, thresh=0.5, save_path=None):
        ...
        if self.binary:
            ...
            A = plot_dependencies(
                d, title=title, scale_values=self.scale_values, sort_rows=self.sort_rows
            )
        else:
            Pos_Feats = {}
    ...
    def _explain_class(
        self, x_raw, x, k, typ="pos", feat_names=None, thresh=0.5, recompute=True
    ):
        """
        Given an input x and class index k, explain f(x) by returning indices of
        features in x that have highest positive impact on predicting class k.
...
def plot_theta_stability(
    model, input, pert_type="gauss", noise_level=0.5, samples=5, save_path=None
):
