        ...
        for i in range(X.shape[0]):
            ...
            imagebox = offsetbox.AnnotationBbox(
                offsetbox.OffsetImage(
                    X[i, :].reshape(28, 28), cmap=plt.cm.gray_r
                ),
                Xp[i],
            )
...
def _explain_class(
    model, x_raw, x, k, typ="pos", thresh=0.5, recompute=True
):
    """
    ...
    if typ == "pos":
        Mask = (
            (B_k > thresh).astype(np.int).reshape(x.size()).squeeze()
        )
    elif typ == "neg":
        Mask = (
            (B_k < -thresh).astype(np.int).reshape(x.size()).squeeze()
...
def explain_digit(model, x_raw, x, thresh=0.5, save_path=None):
    ...
    fig, ax = plt.subplots(
        3, model.dout, figsize=(1.5 * model.dout, 1.5 * 3)
    )
    for i in range(model.dout):
        # print('Class {}:'.format(i))

        # Positive
        x_imask = _explain_class(
            model,
            x_raw,
            x,
            i,
        ...
        # Combined
        x_imask = _explain_class(
            model, x_raw, x, i, typ="both", recompute=False
        )
...
class FeatureInput_Explainer:
    ...
    def explain(self, model, x, thresh=0.5, save_path=None):
        ...
        Pos_Mask = (B > thresh).astype(
            np.int
        )  # .reshape(x.size()).squeeze()
        Neg_Mask = (B < thresh).astype(
            np.int
        )  # .reshape(x.size()).squeeze()

        title = r"Relevance Score $\theta(x)$" + (
            " (Scaled)" if self.scale_values else ""
        )
        if self.binary:
            d = dict(
                zip(self.features, B[:, 0])
            )  # Change to B[0,:] when B model is truly binary
            A = plot_dependencies(
                d,
                title=title,
            ...
            for k in range(B.shape[0]):
                ...
                Neg_Feats = list(
                    compress(self.features, B[k, :] < -thresh)
                )
                Pos_Feats = list(
                    compress(self.features, B[k, :] > thresh)
                )
                print(
                    "Class:{:5} Neg: {}, Pos: {}".format(
                        k, ",".join(Neg_Feats), ",".join(Pos_Feats)
                    )
                )
        if save_path:
            plt.savefig(
                save_path, bbox_inches="tight", format="pdf", dpi=300
            )
        plt.show()
        print("-" * 60)
...
def plot_dependencies(
    ...
    if np.sum(np.array(dictionary_values.values())) == 0.0:
        print(
            "Feature dependence for all attributes equal zero."
    ...
    if scale_values:
        coefficient_values = (
            np.array(coefficient_values) / maximum_value
        ) * 100
    ...
    sorted_column_values = list(
        np.array(coefficient_values)[index_sorted]
    )
    pos = np.arange(len(sorted_column_values)) + 0.7

    # rearrange this at some other point.
    def assign_colors_to_bars(
        array_values,
        ...
        if reverse:
            pos_influence, negative_influence = (
                negative_influence,
                pos_influence,
            )
    ...
    bar_colors = assign_colors_to_bars(
        coefficient_values, reverse=True
    )
    ...
    ax.barh(
        pos, sorted_column_values, align="center", color=bar_colors
    )
    ax.set_yticks(pos)
...
def plot_theta_stability(
    ...
    fig, ax = plt.subplots(
        2, len(inputs), figsize=(2 * len(inputs), 1.5 * 3)
    )
    ...
    for i, x in enumerate(inputs):
        ...
        A = plot_dependencies(
            d, title="Dependencies", sort_rows=False, ax=ax[1, i]
        )
...
def concept_grid(
    ...
    """
    print(
        "Warning: make sure data_loader passed to this function doesnt shuffle data!!"
    )
    all_norms = []
    ...
    top_activations = {
        k: np.array(top_k * [-1000.00]) for k in range(num_concepts)
    }
    ...
    top_activations, top_idxs = torch.topk(
        all_activs, int(top_k / 2), 0
    )
    low_activations, low_idxs = torch.topk(
        -all_activs, int(top_k / 2), 0
    )
    ...
    for i in range(num_concepts):
        ...
        buf = buf.reshape(
            [1, buf.shape[1], buf.shape[2], buf.shape[0]]
        )
        for k in range(3):
            buf[:, :, :, k] = data_loader.dataset[top_idxs[i][0]][0][
                k
            ]
            buf[:, :, :, k] = (
                buf[:, :, :, k] - buf[:, :, :, k].min()
        ...
        for j in range(1, top_k):
            ...
            buf2 = buf2.reshape(
                [1, buf2.shape[1], buf2.shape[2], buf2.shape[0]]
            )
            for k in range(3):
                buf2[:, :, :, k] = data_loader.dataset[
                    top_idxs[i][j]
                ][0][k]
                buf2[:, :, :, k] = (
                    buf2[:, :, :, k] - buf2[:, :, :, k].min()
                ) / (buf2[:, :, :, k].max() - buf2[:, :, :, k].min())
    ...
    fig, axes = plt.subplots(
        figsize=figsize, nrows=num_rows, ncols=num_cols
    )
    ...
    for i in range(num_concepts):
        for j in range(top_k):
            ...
            axes[pos].imshow(
                top_examples[i][j], interpolation="nearest"
            )
            if layout == "vertical":
                axes[pos].axis("off")
                if j == 0:
                    axes[pos].set_title(
                        "Cpt {}".format(i + 1), fontsize=24
                    )
            else:
                ...
                if j == 0:
                    axes[pos].set_ylabel(
                        "Concept {}".format(i + 1), rotation=90
                    )
...
def plot_prob_drop(attribs, prob_drop, save_path=None):
    ...
    ax1.set_ylabel(
        r"Feature Relevance $\theta(x)_i$", color=color1, fontsize=14
    )
    # ax1.set_ylim(-1,1)
