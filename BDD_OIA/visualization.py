...
def convert_concepts_to_indexes(values, reference_array):
    ...
    value_to_index = {value: index for index, value in enumerate(reference_array)}
    # Map the elements of the first array to indices from the second array
    mapped_indices = np.array([value_to_index[value] for value in values])
...
def produce_scatter_multi_class(
    x_values_list, y_values_list, labels, dataset, suffix, colors=None, markers=None
):
    if colors is None:
    ...
    for i, (x_values, y_values) in enumerate(zip(x_values_list, y_values_list)):
        plt.scatter(
            [x_values], [y_values], color=colors[i], marker=markers[i], label=labels[i]
        )
    ...
    file_path = f"{OUTPUT_FOLDER}/{dataset}_hc_ece_scatter_plot{suffix}.png"
    plt.savefig(file_path, dpi=150)
