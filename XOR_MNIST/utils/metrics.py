...
def evaluate_metrics(
    ...
    for i, data in enumerate(loader):
        ...
        if last and i == 0:
            ...
            pc_pred = np.concatenate(
                [pc_pred, out_dict["pCS"].detach().cpu().numpy()], axis=0
            )
        ...
        elif args.dataset in ["kandinsky", "prekandinsky", "minikandinsky"]:
    ...
    if last:
        ...
        if args.dataset not in ["kandinsky", "prekandinsky", "minikandinsky"]:
...
def evaluate_metrics_ensemble(ensemble, loader, args, last=True):
    ...
    y_true = torch.tensor(y_true_all)[:, -1]  # out_dict['LABELS'][:,-1]
    y_pred = torch.argmax(y, dim=-1)

    assert y_pred.size() == y_true.size(), f"size c_pred: {c_pred.size()}"
...
def get_concept_probability_ensemble(models, loader):
    ...
    return calculate_mean_pCs(ensemble_c_prb_1, ensemble_c_prb_2, len(ensemble_c_prb_1))
...
def get_concept_probability_factorized_mcdropout(
    ...
    return avg_c_prb_1, avg_c_prb_2, gt_factorized  # (6000,10), (6000,10)
...
def get_concept_probability_factorized_laplace(
    device, loader, laplace_single_prediction, la, output_classes, num_concepts
):
    ...
    for i, data in enumerate(loader):
        ...
        out_dict = laplace_single_prediction(la, images, output_classes, num_concepts)
...
def get_concept_probability_laplace(device, loader, laplace_model, n_ensembles):
    ...
    for sample_index in range(num_samples):
        ...
        for i, data in enumerate(loader):
            ...
            out_dict = recover_predictions_from_laplace(
                out_tensor, out_tensor.shape[0], output_classes, num_concepts
            )
