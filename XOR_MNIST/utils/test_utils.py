...
def fprint_weights_distance(original_weights, ensemble, method_1, method_2):
...
def compute_concept_factorized_entropy(
    ...
    for c_fact, key in zip(
        [c_fact_1, c_fact_2, c_fact_stacked, p_w_x], ["c1", "c2", "c", "(c1, c2)"]
    ):
...
def produce_confusion_matrices(
    ...
    concept_labels = [
        "".join(comb) for comb in product([str(el) for el in range(10)], repeat=2)
    ]
    concept_labels_single = [
        "".join(comb) for comb in product([str(el) for el in range(10)])
    ]
