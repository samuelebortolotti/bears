# This module contains stuff useful for debugging purposes
...
OUTPUT_FOLDER = "./plots"  # PLOTS FOLDER


def create_output_folder() -> None:
    """Method which creates the output folder, if it does not exists
...
def produce_confusion_matrix(
    ...
    """This function produces a confusion matrix.

    Args:
        title (str): title of the figure
        y_true (ndarray): groundtruth values
        y_pred (ndarray): predictions
        labels (List[str]): labels
        plot_title (str): title of the plot
        normalize (str, default="all"): normalization strategy
        ntimes (int): how many tick intervals

    Returns:
        None: This function does not return a value.
    """

    create_output_folder()  # create output folder if it does not exists
    ...
    all_labels = np.union1d(np.unique(ground_truth), np.unique(predictions))
    ...
    np.save(f"{OUTPUT_FOLDER}/{plot_title}.npy", cm)
    ...
    tick_labels = [x_labels[i] if i % ntimes == 0 else "" for i in range(len(x_labels))]
    ax.set_xticks(tick_positions, tick_labels, rotation=90, fontsize=10)

    y_labels = ax.get_yticklabels()
    tick_positions = np.arange(len(y_labels))
    tick_labels = [y_labels[i] if i % ntimes == 0 else "" for i in range(len(y_labels))]
...
def produce_world_probability_table(
    ...
    """This function produces a table containing the probability of each world.

    Args:
        data_dict (Dict[str, float]): world-probability dictionary
        title (str): title of the plot
        key_string (str): world label
        key_value (str): probabilities label
    ...
    table_data = [[key, f"{value:.2f}"] for key, value in zip(keys, values)]
    table = ax.table(
        cellText=table_data,
    ...
    ax.set_title(title, fontweight="bold", fontsize=16, color="#3366cc")
...
def produce_alpha_matrix(
    ...
    """This function produces the alpha matrix.

    Args:
        data_dict (Dict[str, float]): world-probability dictionary
        title (str): title of the figure
        concept_labels (List[str]): concept labels
        plot_title (str): title of the plot
    ...
    plt.xticks(np.arange(len(concept_labels)), concept_labels, rotation=90, fontsize=10)
...
def produce_scatter_multi_class(
    ...
    """This function produces a multi-class scatter plot

    Args:
        x_values_list (List[float]): x values
        y_values_list (List[float]): y values
        labels (List[str]): labels
        dataset (str): dataset name
        xlab (str): label to put on the x axis
        title_extra (str): extra part of the title
        name (str): name of the plot
        colors (List[str], default=None): colors for the plot
        markers (List[str], default=None): markers for the plot
    ...
    # Customize the plot
    plt.title(f"Scatter Plot Dataset: {dataset} {title_extra}")
    plt.xlabel(xlab)
    ...
    file_path = f"{OUTPUT_FOLDER}/{dataset}_{name}.png"
    plt.savefig(file_path, dpi=150)
...
def plot_grouped_entropies(
    ...
    """This function produces a grouped bar plot for showing entropies

    Args:
        categories (List[str]): categories of the bar plot
        dataset (str): dataset name
        values_list (List[float]): lists of values
        group_labels (List[str]): name of the groups
        prefix (str): prefix of the plot title
        title (str): title of the plot
        save (bool, default=True): whether to save the plot or not
        ax (default=None): axes where to plot
        fig (default=None): figure where to save
        add_numbers (bool, default=False): whether to add numbers to the bars
        set_lim (bool, default=False): whether to set lims on the plot or not (0, 1)
    ...
    total_group_width = num_categories * bar_width + (num_categories - 1) * group_gap
    ...
    if save:
        file_path = f"{OUTPUT_FOLDER}/{dataset}_{prefix}_hc_bar_plot.png"
        fig.tight_layout()
        plt.savefig(file_path, dpi=150)
...
def produce_calibration_curve(
    ...
    """This function produces a calibration curve plot

    Args:
        bin_info (Dict[str, float]): value per bin
        ece (float): ece value
        plot_title (str, default="calibration_curve"): title of the plot
        concept (bool, default=True): whether it is computed on concepts or labels
    ...
    bin_confidence = [bin_info[i]["BIN_CONF"] for i in range(num_bins)]
    ...
    ax1.axvline(
        x=avg_confidence, color="red", linestyle="--", label="Weighted Avg. Confidence"
    )
    ax1.axvline(
        x=avg_accuracy, color="black", linestyle="--", label="Weighted Avg. Accuracy"
    )
...
def produce_bar_plot(
    ...
    """This function produces a bar plot

    Args:
        data (ndarray): data to plot
        xlabel (str, default="Groundtruth class"): name of the x label
        ylabel (str, default="Occurrences"): name of the y label
        title (str, default=""): title of the figure
        plot_name (str, default="bar_plot"): name of the plot
        ylim (bool, default=False): whether to put ylimits (0, 1)
        x_ticks (List[str], default=[]): xticks
    ...
    plt.bar(indices, data, color="blue", edgecolor="black", linewidth=1.2)
