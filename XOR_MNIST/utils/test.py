...
def test(model: MnistDPL, dataset: BaseDataset, args, **kwargs):
    ...
    mean_h_c, yac, cac, cf1, yf1 = print_metrics(
        y_true, y_pred, c_true, c_pred, p_cs_all, model.n_facts, args.type
    )
    ...
    if args.type == EVALUATION_TYPE.NORMAL.value:
        worlds_prob, c_factorized_1, c_factorized_2, worlds_groundtruth = (
            get_concept_probability(model, test_loader)
        ...
        ece = produce_ece_curve(
            p_c_normal, c_pred_normal, worlds_groundtruth, args.type, "concepts"
        )
    ...
    if args.type == EVALUATION_TYPE.MC_DROPOUT.value:
        ...
        # Obtain the factorized probabilities
        c_factorized_1, c_factorized_2, gt_factorized = (
            get_concept_probability_factorized_ensemble(ensemble, test_loader)
        )
    elif args.type == EVALUATION_TYPE.LAPLACE.value:
    ...
    if not any(
        key in kwargs for key in ["c_acc_count", "c_acc", "w_acc_count", "w_acc"]
    ):
