...
def get_label(c1, c2, labels, args):
    if args.task == "addition":
    ...
    elif args.task == "product" and not args.model in [
        "mnistltn",
        "mnistltnrec",
    ]:
        n_queries = [0]
        ...
        prod = c1 * c2
        q = [
            np.argmax(np.array(n_queries == p, dtype=int))
            for p in prod
        ]
        q = np.array(q)
    ...
    elif args.task == "product" and args.model in [
        "mnistltn",
        "mnistltnrec",
    ]:
        return c1 * c2

    elif args.task == "multiop" and not args.model in [
        "mnistltn",
        "mnistltnrec",
    ]:
    ...
    elif args.task == "multiop" and args.model in [
        "mnistltn",
        "mnistltnrec",
    ]:
...
class nMNIST(Dataset):
    ...
    def reset_counter(self):
        self.world_counter = {
            c: self.samples_x_world // self.batch_size
...
def create_dataset(
    ...
    MNIST = datasets.MNIST(
        root="./data/raw/", train=train, download=download
    )
    ...
    for k, v in tqdm(label2idx.items()):
        for i, label in enumerate(labels):
            if tuple(label[:2]) == k:
                v.add(i)
    label2idx = {
        k: torch.tensor(list(v)) for k, v in label2idx.items()
    }
...
def check_dataset(n_digits, data_folder, data_file, dataset_dim):
    ...
    try:
        ...
        samples_x_world = {
            k: int(d / n_worlds) for k, d in dataset_dim.items()
        }
        dataset_dim = {
            k: s * n_worlds for k, s in samples_x_world.items()
        }

        train_imgs, train_labels, train_indexes = create_dataset(
            n_digit=n_digits,
            sequence_len=2,
            samples_x_world=samples_x_world["train"],
            train=True,
            download=True,
        )

        val_imgs, val_labels, val_indexes = create_dataset(
            n_digit=n_digits,
            sequence_len=2,
            samples_x_world=samples_x_world["val"],
            train=True,
            download=True,
        )
        test_imgs, test_labels, test_indexes = create_dataset(
            n_digit=n_digits,
            sequence_len=2,
            samples_x_world=samples_x_world["test"],
            train=False,
            download=True,
        )
        ...
        for key, value in indexes.items():
            torch.save(
                value, os.path.join(data_folder, f"{key}_indexes.pt")
            )
...
def load_2MNIST(
    ...
    data_folder = os.path.join(
        data_folder, f"2mnist_{n_digits}digits"
    )
    data_file = f"2mnist_{n_digits}digits.pt"

    # Check whether dataset exists, if not build it
    check_dataset(
        n_digits, data_folder, data_file, dataset_dimensions
    )
...
def generate_r_seq(size):
    # fixed since the seed is fixed
