...
class DeepProblogModel(nn.Module):
    ...
    def __init__(
        self, encoder, model_dict=None, n_facts=20, nr_classes=19
    ):
        super(DeepProblogModel, self).__init__()
        self.encoder = encoder
        self.model_dict = (
            model_dict  # Dictionary of pre-compiled ProbLog models
        )
    ...
    def forward(self, x):
        ...
        self.query_prob, self.worlds_prob = self.problog_inference(
            self.facts_probs
        )
